...
import random
import pickle
from day15.atmpickle.card import Card
from day15.atmpickle.user import User


class ATM:
    @staticmethod
    def saveDict2File(dic):
        # 由于每次都是将内存中一个完整的字典保存，所以不用'ab'而用'wb'，这样文件才不会越变越大
        with open('atmdb.txt', 'wb') as f:
            pickle.dump(dic, f)

    @staticmethod
    def loadATMDict():
        with open('atmdb.txt', 'rb') as f:
            dbDict = {}
            while True:
                try:
